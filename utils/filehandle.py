...
# 将分词后的结果输出到文件
def writeResult(outputFile, contents_after):
    with open(outputFile, 'w', encoding='utf-8') as outfile:
        outfile.writelines(contents_after)


#评分程序  file1 标准文件 file2 待评测文件
#评分 F = 15 * 2 * Precision * Recall /（Precision + Recall）
def checkFile(file1, file2):
    ...
    for i in range(len(content1)):
        list1 = content1[i].rstrip().split()
        listval1 = [0 for val in list1]
        list2 = content2[i].split()
        listval2 = [0 for val in list2]
        dict1 = dict(zip(list1,listval1))
        dict2 = dict(zip(list2,listval2))

        for j in dict1.keys():
            if j in dict2.keys():
                correct+=1
        std_count +=len(dict1)
        model_count += len(dict2)
    P = correct/model_count
    R = correct/std_count
    F = 2*P*R /(P+R)
    print("词总数：%d" % (model_count))
    print("正确词总数：%d" % (correct))
    print("P:%f" % (P))
    print("R:%f" % (R))
    print("分数：%f" %(15 * F))

if __name__ == '__main__':
    # deletespace("../train/test.txt","../train/test_final.txt")
    # filelstrip("train_1.txt","train_2.txt")
    # fileSplit('source.txt','train.txt','test.txt',0.7)
    checkFile("../train/train.txt", "../train/2017111457.txt")
    checkFile("../train/test.txt", "../train/test_result.txt")
